...
export default function StandupReports() {
    ...
    return (
        ...
        >
            {/* Fixed Header - Similar to TaskPage design */}
            <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-white/20 shadow-lg p-4">
            ...
            {loading ? (
                <div className="flex justify-center py-20">
                    <div className={`animate-spin rounded-full h-12 w-12 border-b-2 ${
                        reportsViewMode === 'today' ? 'border-orange-500' : 'border-indigo-600'
                    }`}></div>
                </div>
            ) : filteredReports.length === 0 ? (
                <div className={`text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed ${
                    reportsViewMode === 'today' ? 'border-orange-200' : 'border-gray-200'
                }`}>
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${
                        reportsViewMode === 'today' ? 'bg-orange-100 text-orange-500' : 'bg-gray-100 text-gray-400'
                    }`}>
                ...
                <div className="grid grid-cols-1 gap-6">
                    {filteredReports.map((report, index) => (
                        <motion.div
                            ...
                            className={`bg-white rounded-2xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow relative group ${
                                reportsViewMode === 'today' ? 'border-orange-100' : 'border-gray-100'
                            }`}
                        >
                            {/* Maximize Button */}
                            <motion.button
                    ...
                    <AnimatePresence initial={false} custom={slideDirection} mode="wait">
                        <motion.div
                            ...
                            className={`bg-white rounded-2xl shadow-lg overflow-hidden ${
                                reportsViewMode === 'today' ? 'border border-orange-100' : 'border border-gray-100'
                            }`}
                        ...
                        >
                            ...
                            <div className="p-8">
                                ...
                                <div className="grid md:grid-cols-3 gap-8">
                                    <div className={`rounded-xl p-6 shadow-sm h-[300px] flex flex-col hover:shadow-md transition-all duration-300 ${
                                        reportsViewMode === 'today'
                                            ? 'bg-orange-50 border border-orange-100'
                                            : 'bg-gray-50 border border-gray-100'
                                    }`}>
                                        <h4 className={`font-semibold mb-3 flex items-center justify-center text-sm border-b pb-2 ${
                                            reportsViewMode === 'today'
                                                ? 'text-orange-700 border-orange-200'
                                                : 'text-gray-700 border-gray-200'
                                        }`}>
                                            <span className={`h-5 w-5 rounded-full flex items-center justify-center mr-2 text-xs font-bold ${
                                                reportsViewMode === 'today'
