        ...
        switch (itemType) {
            ...
            default:
                return {
                    ...
                    hoverBg: 'hover:bg-gray-100 dark:hover:bg-slate-800'
                };
        }
    };
    ...
    return (
        <motion.div
            ...
            className={`group flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all duration-200 relative ${active
                ? `${colorConfig.bg ? colorConfig.bg + ' ' : ''}${colorConfig.text} font-medium shadow-sm ${colorConfig.hoverBg || 'hover:bg-gray-100 dark:hover:bg-slate-800'}`
            ...
            {editMode && (
                <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    ...
                    <Menu as="div" className="relative">
                        ...
                        <Menu.Items className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-slate-900 rounded-lg shadow-lg dark:shadow-slate-950/80 border border-gray-100 dark:border-slate-800 py-1 z-50 focus:outline-none transition-all duration-200">
                            <Menu.Item>
                                {({ active }) => (
                                    <button
                                        onClick={onRename}
                                        className={`${active ? 'bg-gray-50 dark:bg-slate-700' : ''} flex items-center gap-2 w-full px-3 py-1.5 text-xs text-gray-600 dark:text-gray-300`}
                                    >
                            ...
                            <Menu.Item>
                                {({ active }) => (
                                    <button
                                        onClick={onDelete}
                                        className={`${active ? 'bg-red-50 dark:bg-red-900/20' : ''} flex items-center gap-2 w-full px-3 py-1.5 text-xs text-red-600 dark:text-red-400`}
                                    >
...
const ProjectSidebar = ({
    ...
    return (
        ...
        >
                ...
                {isOpen && (
                    <div className="flex items-center gap-2 text-gray-500 bg-white dark:bg-slate-950/50 border border-gray-200 dark:border-slate-800 px-3 py-2 rounded-lg shadow-sm w-full focus-within:ring-2 focus-within:ring-indigo-100 dark:focus-within:ring-indigo-900/30 transition-all">
                        ...
                        <input
                            type="text"
                            placeholder="Search..."
                            className="bg-transparent border-none outline-none text-sm w-full placeholder-gray-400 dark:text-white"
                        />
                    </div>
                )}
            ...
            {/* Navigation Tree */}
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-slate-700">
                {isOpen ? (
                    <div className="px-3 py-2 space-y-1">
                        ...
                        {sections.length === 0 && editMode && (
                            <div className="text-center py-8 px-4">
                                ...
                                <button
                                    onClick={onAddSection}
                                    className="w-full py-2 border border-dashed border-gray-300 dark:border-slate-700 rounded-lg text-xs text-gray-500 dark:text-gray-400 hover:border-indigo-300 dark:hover:border-indigo-700 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                                >
                                    Create Section
                                </button>
                            </div>
                            ...
                            <React.Fragment key={section.id}>
                                <button
                                    className={`p-2 rounded-lg w-8 h-8 flex items-center justify-center transition-colors ${section.project_topics?.some(topic =>
                                        selectedTopic?.id === topic.id ||
                                        (selectedContent && selectedContent.topic_id === topic.id)
                                    ) ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/20 dark:text-rose-400' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-700'
                                        }`}
                                    title={section.name}
                                    onClick={() => {
                                            ...
                                            {section.project_topics?.map(topic => (
                                                <React.Fragment key={topic.id}>
                                                    <div className="flex items-center justify-center gap-1">
                                                        <button className={`p-2 rounded-lg w-8 h-8 flex items-center justify-center transition-colors mx-auto ${selectedTopic?.id === topic.id || (selectedContent && selectedContent.topic_id === topic.id)
                                                        ...
                                                        {editMode && onAddContent && (
                                                            <button
                                                                className="p-1 rounded w-6 h-6 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-700"
