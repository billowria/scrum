...
import { 
  FiUsers, FiSearch, FiUser, FiMail, FiBriefcase, FiPhone, 
  FiHash, FiLinkedin, FiCalendar, FiX, FiEdit2, FiSave, FiAlertCircle
} from 'react-icons/fi';

const ManagerUserProfile = () => {
  const [currentUser, setCurrentUser] = useState(null);
  const [teamId, setTeamId] = useState(null);
  const { teamMembers, loading, error, updateTeamMember, refreshTeamMembers } = useTeamProfiles(teamId);
      ...
      <div className="bg-yellow-50 p-6 rounded-lg border border-yellow-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-yellow-700 text-sm">You must be a manager to access this page.</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="relative w-full md:w-80">
              <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-200 w-5 h-5" />
              <input
                type="text"
                placeholder="Search team members..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-10 pr-4 py-3 bg-white/20 border border-white/30 rounded-lg text-white placeholder-indigo-200 focus:outline-none focus:ring-2 focus:ring-white/50"
              />
              {searchTerm && (
                <button
